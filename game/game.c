...
void game(MPI_Comm comm_2D, int my_rank, int NPROWS, int NPCOLS, int MAX_GENS)
{
    ...
    int gen, i, j;
    next_generation = allocate_memory(local_N + 2, local_M + 2);
    char **temp;
    ...
    MPI_Send_init(&(local_matrix[1][local_M]), 1, MPI_CHAR, topright, MESSAGE_TAG, comm_2D, &array_of_requests[5]);
    MPI_Send_init(&(local_matrix[local_N][local_M]), 1, MPI_CHAR, bottomright, MESSAGE_TAG, comm_2D, &array_of_requests[6]);
    MPI_Send_init(&(local_matrix[local_N][1]), 1, MPI_CHAR, bottomleft, MESSAGE_TAG, comm_2D, &array_of_requests[7]);
    ...
    MPI_Recv_init(&(local_matrix[0][local_M + 1]), 1, MPI_CHAR, topright, MESSAGE_TAG, comm_2D, &array_of_requests[13]);
    MPI_Recv_init(&(local_matrix[local_N + 1][local_M + 1]), 1, MPI_CHAR, bottomright, MESSAGE_TAG, comm_2D, &array_of_requests[14]);
    MPI_Recv_init(&(local_matrix[local_N + 1][0]), 1, MPI_CHAR, bottomleft, MESSAGE_TAG, comm_2D, &array_of_requests[15]);
